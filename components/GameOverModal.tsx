...
export const GameOverModal: React.FC<GameOverModalProps> = ({ winnerName, onGoHome, t }) => {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
      <div className="bg-gray-800 border-2 border-cyan-500 rounded-lg p-8 shadow-2xl shadow-cyan-500/40 text-center">
        <h2 className="text-3xl font-bold text-cyan-400 mb-4">{t('gameOver.title')}</h2>
        <p className="text-xl text-white mb-6">
          <span className="font-bold text-yellow-300">{winnerName}</span> {t('gameOver.victoryMessage', { winnerName })}
        </p>
        <button
          onClick={onGoHome}
